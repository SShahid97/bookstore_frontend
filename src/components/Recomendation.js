...
import {Item_Service,Order_Service} from '../services/Service';
import Rating from "./Rating";

function Recomendation({user}) {
    ...
    useEffect(()=>{
        // console.log(cust);
        getOrderHistory(cust.token, cust._id);
    },[]);
          ...
          if(response.status === 200){
            ...
            uniqueBookWithQty.forEach((item,index)=>{
              for(let i=index+1; i<uniqueBookWithQty.length; i++){
                if(item.category === uniqueBookWithQty[i].category){
            ...
            let category = uniqueBooksArr[0].category;
            getRecommendedBooks(category);
            // console.log(uniqueBooksArr[0].category)
    ...
    const getRecommendedBooks = async(cat)=>{
        const response = await Item_Service.getBooksByCategory(cat);
        if(response.status === 200){
    ...
    {hasOrders && (
    <Wrapper >
         <h3 style={{fontWeight: '500'}}>Recommended books for you</h3>
    <Splide
        ref={ref}
        options={{
            arrows:true,
